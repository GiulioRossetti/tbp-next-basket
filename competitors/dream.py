...
def main():
    ...
    dream = DREAM(n_user, n_item, alpha, decay, lambda_r, verbose, min_nbr_baskets, n_hidden,
                  embedding_dimension, n_epoch, withbpr, continue_f, isrank)

    dream.build_model(customers_baskets, path)
    ...
    for customer_id in customers_train_set:
        ...
        pred_basket = dream.predict(user_id, pred_length)
    ...
    print datetime.datetime.now(), 'DREAM', 'avg', stats['avg']
    print datetime.datetime.now(), 'DREAM', 'iqm', stats['iqm']
